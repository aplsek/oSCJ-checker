...
/**
 * A clock marks the passing of time. It has a concept of now that can be queried
 ...
 * other clocks are not valid for any purpose that involves sleeping or waiting,
 * including in members of the RealtimeThread.waitForNextPeriod() family. They may,
 * however, be used in the fire time and the period of OneShotTimer and PeriodicTimer. 
 */
@SCJAllowed
public abstract class Clock {
	 ...
	 * @return A newly allocated instance of AbsoluteTime in the current allocation context, representing the current time. The returned object is associated with this clock.
	 */
	@Allocate( { CURRENT })
	@BlockFree
	@SCJAllowed
	 ...
	 * @param dest The instance of AbsoluteTime object which will be updated in place. The clock association of the dest parameter is ignored. When dest is not null the returned object is associated with this clock. If dest is null, then nothing happens.
	 * @return The instance of AbsoluteTime passed as parameter, representing the current time, associated with this  clock, or null if dest was null.
	 */
	@BlockFree
	@SCJAllowed
	 ...
	 * 
	 * @param dest return the relative time value in dest. If dest is null, allocate a new RelativeTime instance to hold the returned value.
	 * @return dest set to values representing the resolution of this. The returned object is associated with this clock.
	 */
	@BlockFree
	@SCJAllowed
	 ...
	 * @return A newly allocated RelativeTime object in the current execution context with the offset past the Epoch for this clock. The returned object is associated with this clock.
	 */
	@Allocate( { CURRENT })
	@BlockFree
	@SCJAllowed
	...
	/**
	 * Returns true if and only if this Clock is able to trigger the execution of
	 * time-driven activities. Some user-defined clocks may be read-only, meaning
	 * the clock can be used to obtain timestamps, but the clock cannot be used to
	 * trigger the execution of events. If a clock that does not return drivesEvents()
	...
	/**
	 * Code in the abstract base Clock class makes this call to the subclass. The method is expected to implement a mechanism that will invoke atTime()  in ClockCallBack at time time, and if this clock is subject to discontinuities, invoke ClockCallBack.discontinuity(javax.realtime.Clock, javax.realtime.RelativeTime) each time a clock discontinuity is detected. 
	 *
	 * This method behaves effectively as if it and invocations of clock events by this clock hold a common lock. 
	 * @param time The absolute time value on this clock at which ClockCallBack.atTime(Clock) should be invoked.
	 * @param clockEvent The object that should be notified at time. If clockEvent  is null, unregister the current clock event. 
	 */
	@BlockFree
	@SCJAllowed(LEVEL_1)
