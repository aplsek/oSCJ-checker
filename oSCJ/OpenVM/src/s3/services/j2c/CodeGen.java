...
public class CodeGen extends Runabout {
	...
	static final boolean EMIT_LINE_NUMBERS = true; // changed so that I can
													// debug j2c

	static final boolean DISABLE_LINE_CONTINUATION = true;

	/**
	 * If false, avoid emitting a #line directive for the `next' line. This
	 * should make the .c file smaller, and potentially make compilation faster,
	 * but it also makes the .c file less readable.
	 **/
	static final boolean EMIT_ALL_LINE_NUMBERS = true;

	/**
	 * pc comments don't work well with line numbers and gcc 3.0.2: gcc -E will
	 * ignore the newline escape for lines containing comments immediately after
	 * #line directives.
	 * 
	 * Anyway, the pc of a push statement is clear from the left-hand-side, and
	 * the start pc of a block is clear from its label.
	 **/
	static final boolean EMIT_PC_COMMENTS = false;

	static final String TERMINATOR = ((EMIT_LINE_NUMBERS && (!DISABLE_LINE_CONTINUATION)) ? " \\"
	...
	// "w" can be switched by "setAfterEndPrintWriter" and
	...
	void ptype(S3Blueprint bp) {
		if (bp.isScalar()) {
			...
			w.print("*");
			w.print(" /* ");
			w.print(J2cFormat.format(((S3Blueprint.Array) bp)
	...
	/**
	 * exists to decide that the finally clause should be implemented as a catch
	 * of java_lang_Object, which it sometimes is in PARs, or should be
	 * implemented as a catch of java_lang_Throwable, which is what we have been
	 * doing all along.
	 * 
	 * You will notice that where there used to be strings like "catch
	 * e_java_lang_Object *o" in the rest of this code, there are now calls to
	 * this method "finallyReplacement("o", "e", false);
	 * 
	 * PARBEGIN PAREND
	 * 
	 * @param varName
	 *            the variable name for the caught object
	 * @param typeContext
	 *            Executive or User Domain
	 * @param translateThrowable
	 * @return code that opens a catch clause and binds varName either
	 *         <blockquote><code>
	 * catch (<i>d</i>_java_lang_Object *o) {
	 *    <i>d</i>_java_lang_Throwable *<i>varName</i> = (<i>d</d>_java_lang_Throwable *) o;
     * </code></blockquote> or <blockquote><code>
     * catch(<i>d</i>_java_lang_Throwable *<i>varName</i>) {
     * </code></blockquote>
	 */
	String finallyReplacement(String varName, String typeContext,
			boolean translateThrowable) {
		...
		if (Transaction.the().transactionalMode()
				...
				|| ((!Transaction.the().isTransMethod(m))
						&& (!Transaction.the().isAtomic(m)) && (!inAEBlock))) {
			return "catch (" + objectString + " *o) { " + throwableString
					+ " *" + varName + " = (" + throwableString + " *) o;";
		} else {
	...
	void emitBarrierSetup() {
		...
		if (ctx.doBarrierProf) {
			w.println("#define BEGIN_" + side
					+ "_BARRIER __asm__ __volatile__ (\"# begin barrier\")");
			w.println("#define END_" + side
					+ "_BARRIER __asm__ __volatile__ (\"# end barrier\")");
		} else {
	...
	/**
	 * Return true if it is not safe to use the variable var in the C++ formal
	 * parameter list with type formalType. This may happen because we never
	 * bother to construct a J2cValue for an unused parameter, or we have
	 * inferred a different type for the parameter, or it is a pointer that must
	 * be copied to a GCFrame structure.
	 **/
	boolean mustRenameParameter(J2cValue var, Type formalType) {
		...
		return (var == null || var.kind != J2cValue.LOCAL_VAR
				...
				|| (LocalAllocator.flavorTag[var.flavor] != fTag && fTag != TypeCodes.ARRAY) || (ctx.gcFrames
				&& mc.everLive.get(var.number) && !currentCallsiteIgnoresSafePoints));
	}
	...
	CodeGen(MethodCompiler mc, PrintWriter w) {
		...
		if (PragmaIgnoreExceptions.descendantDeclaredBy(mc.meth.getSelector(),
				ctx.blueprintFor(mc.meth.getDeclaringType())) != null) {
		...
		if (PragmaIgnoreSafePoints.descendantDeclaredBy(mc.meth.getSelector(),
				ctx.blueprintFor(mc.meth.getDeclaringType())) != null) {
		...
		if (RAPITA) {
			// w.println("typedef unsigned short rapita_t;");

			w.println("#define RAPITA_BASE 0x80000800");
			w.println("#define RAPITA_OUT (*(rapita_t *) (RAPITA_BASE + 0x4))");
			w.println("#define RAPITA_MASK (*(rapita_t *) (RAPITA_BASE + 0x8))");
			w.println("#define RAPITA_INTERRUPT (*(rapita_t *) (RAPITA_BASE + 0xc))");
		...
		/*
		 * the interrupt handler detection is presently not used... if this
		 * stays for a while, it can be removed
		 */
		boolean isInterruptHandler = false;

		if (ctx.interruptHandlerBP != null) {
			isInterruptHandler = meth.getDeclaringType().isSubtypeOf(
					ctx.interruptHandlerBP.getType());
		}
		...
		w.print("static ");
		if (PragmaInline.declaredBy(meth.getSelector(), ctx.blueprintFor(meth
				.getDeclaringType()))
		...
		for (int i = 0; i < allVars.length; i++) {
			if (allVars[i] != null
					&& (allVars[i].kind != J2cValue.STACK_SLOT || mc.falseLiveness
							.get(allVars[i].number))
		...
		if (mc.maxMultiArray > 0) {
			...
			if (MemoryManager.the().usesArraylets()) {
				assert (mc.maxMultiArray * MachineSizes.BYTES_IN_WORD * 2 < MemoryManager
						.the().arrayletSize());
		...
		// Filip's hacking of pointer lists onto the old Henderson approach.
		// this
		...
		if (ctx.gcFrames) {
			if (mc.everLive.nextSetBit(0) == -1
				...
				// NO - if the code was correct before, it should be now as
				// well, as stack
				// allocated variables are not guaranteed to be zeroed

				/*
				 * zero all references and arrays in frames, otherwise the write
				 * barrier might try to mark them (and crash) let's hope that
				 * GCC optimizes these out when the value is not used
				 */
				/*
				 * for (int i = 0; i < allVars.length; i++) if (allVars[i] !=
				 * null && mc.everLive.get(allVars[i].number)) { S3Blueprint bp
				 * = allVars[i].getBlueprint(dom);
				 * 
				 * if (bp.isReference()) { w.print("frame->");
				 * w.print(allVars[i].getName()); w.print(" = "); w.print("(");
				 * if (bp.getType() instanceof Type.Array) {
				 * w.print(CxxFormat.formatCArray((S3Blueprint.Array)bp)); }
				 * else { w.print("HEADER"); } w.print(" *)");
		...
		if (ctx.safePointMacros
				&& (outerTranslate || cf.getExceptionHandlers().length > 0)
		...
		for (int i = 0, index = 1; i < meth.getArgumentCount(); i++) {
			try {
				...
				if (!argLocals.get(var.number)) {
					visit(var);
					w
							.print((bp == ctx.VM_AddressBP || bp == ctx.VM_WordBP) ? " = (jint) _arg"
									: " = _arg");
					w.print(i);
					terminateAndIndent(";");
					if (var.flavor == LocalAllocator.REF_VAR)
		...
		// before init
		for (int i = mc.falseLiveness.nextSetBit(0); i != -1; i = mc.falseLiveness
				.nextSetBit(i + 1)) {
			visit(allVars[i]);
		...
		for (bidx = 0; bidx < blocks.length; bidx++) {
			...
			for (; j < curBlock.code.length; j++) {
				...
				if (e.dest != null) {
					...
					if (ctx.gcFrames && mc.everLive.get(e.dest.number)
							&& !currentCallsiteIgnoresSafePoints)
						w.print("frame->");
					// w.print("frame.");
					...
					if (e.dest instanceof J2cReference) {
						...
						if (destbp.isScalar()) {
							...
							w.print(" ((");
							w.print(CxxFormat
									.formatCArray((S3Blueprint.Array) destbp));
							w.print(" *)");
							rest = ")";
						}
					}
		...
		if (ctx.cExceptions) {
			...
			if (outerTranslate) {
				...
				if (!dom.isExecutive()) {
					w
							.println("myExcDom=curExcDom; myExc=curExc; goto outerTranslate2");
				} else {
					try {
						if (meth.getReturnType() != dom.VOID) {
							w.println("return 0");
		...
		if (ctx.cExceptions) {
			...
			if (usedTopNpeThrower) {
				...
				// (the exception would not be reported to be thrown on the
				// correct line)
				// if needed, this can be fixed by a global variable that would
				// hold __LINE__,
		...
		if (usedEndOfMethod) {
			terminate("_end_of_method: ;"); // the semicolon is here to make GCC
											// happy
		}
	...
	/**
	 * Compile global helper functions. Mostly, these functions are used to
	 * assign well-known names to java methods (whose c++ names have a weird
	 * suffix).
	 **/
	CodeGen(final PrintWriter w, J2cImageCompiler cmp) {
		...
		try {
			...
			TypeName.Scalar cfN = RepositoryUtils.makeTypeName(
					"s3/services/j2c", "J2cCodeFragment");
			S3Blueprint _cfBP = (S3Blueprint) exec.blueprintFor(cfN
					.asCompound(), exec.getSystemTypeContext());
			cfBP = (S3Blueprint) _cfBP.getSharedState().getBlueprint();
			if (ctx.safePointMacros) {
				TypeName.Scalar lrtTN = RepositoryUtils.makeTypeName(
						"s3/services/memory/precise",
						"PreciseReferenceIterator");
				lrtBP = (S3Blueprint) exec.blueprintFor(lrtTN.asCompound(),
		...
		S3Method mainMethod = (S3Method) mainType.getMethod(mainBC
				.getSelector());
		w.println("void boot() {");
		...
		if (ctx.noCppExceptions) {
			w
					.println("HEADER *getCurrentException() { return (HEADER*)curExc; }");
			w
					.println("e_java_lang_Error *j2c_throw(HEADER *o) { setExc((void*)o,domainForBlueprint(HEADER_BLUEPRINT(o))); }");
			w
					.println("#define j2c_throw(_o) ({ HEADER *o=_o; setExc((void*)o,domainForBlueprint(HEADER_BLUEPRINT(o))); EXCEPTION_JUMP; (e_java_lang_Error*)0; })");

		} else if (ctx.cExceptions) {

			// although j2c_throw is now translated to a macro
			// in the code generated by j2c, it could be
			// called from other modules ; if called directly, it would not
			// work, since
			...
			if (false)
				w.println("\tubprintf(\"in j2c_throw: %p\\n\",o);");
			w
					.println("\tswitch (HEADER_BLUEPRINT(o)->ovm_ctxNumber << 16 | HEADER_BLUEPRINT(o)->ovm_number) {");
			cmp.new BlueprintWalker() {
				...
				public void beforeDomain(S3Domain d) {
					...
					try {
						throwableBP = (S3Blueprint) d.blueprintFor(tName, sCtx);
					} catch (LinkageException e) {
						throw e.unchecked();
					}
				}
				...
				public void walkBlueprint(S3Blueprint bp) {
					if (bp.isSubtypeOf(throwableBP)
							|| bp.asTypeName().equals(
									Transaction.ED_ABORTED_EXCEPTION)
							|| bp.asTypeName().equals(
									Transaction.ABORTED_EXCEPTION)) { // PARBEGIN
																		// PAREND
																		// FIXME...
						w.print("\tcase ");
		...
		if (DEBUG) {
			w
					.println("\tubprintf(\"Generate throwable called for code %d\\n\",code);");
		}
		w.println("\tsigprocmask(SIG_SETMASK, &j2cNormalMask, 0);");
		if (DEBUG) {
			w
					.println("\tubprintf(\"Generate throwable called for code %d (1)\\n\",code);");
		}
		w.print("\t");
		w.print(J2cValue.makeNonvirtualReference(met).getName());
		w.println("(0, code);");
	 ...
	 * &lt;domain&gt;_signalEvent(). This noninlinable function is called
	 * whenever CHECK_EVENTS() returns true, and is responsible for setting up
	 * native-level event handling state and calling pollingEventHook() on the
	 * appropriate CoreServicesAccess object
	 **/
	public CodeGen(PrintWriter w, Context ctx) {
		...
		if (dom.isExecutive()) {
			...
			w.println("\tif (!" + PollcheckManager.getSettings().slowPathInC()
					+ ") return 0;");
			MethodReference hook = J2cValue
					.makeNonvirtualReference(ctx.pollingEventHook);
	...
	void startTry(BBSpec block, TryBlock t) {
		if (t != null && t.firstBlock == block) {
			...
			if (ctx.cExceptions) {
				...
				terminateAndIndent("");
				if (DEBUG) {
					terminateAndIndent("/* registered start of try block no. "
							+ lastTryBlock + " */");
				}
				if (ctx.safePointMacros) {
	...
	void endTry(BBSpec block, TryBlock t) {
		if (t != null && t.lastBlock == block) {
			...
			if (ctx.cExceptions) {
				// emit the code to translate exceptions and start their
				// dispatch
				...
				try {
					...
					if (DEBUG) {
						terminateAndIndent("/* registered end of try block no. "
								+ currentTryBlock + " */");
					}
					...
					if (ctx.safePointMacros) {
						...
						if (tryBlocks.size() == 0) {
							...
							int prevBlock = ((TryBlockInfo) tryBlocks
									.lastElement()).code;
							w.print(prevBlock);
						}
					...
					// j2c internal error
					throw new Error(
							"try block stack is empty when generating try dispatch code!");
				}
				...
				if (!dom.isExecutive()) {
					...
					// exception translation is done in user domain code and
					// translates
					...
					if (DEBUG) {
						w
								.print("ubprintf(\"Translation exception block with code ");
						w.print(tryCode);
					...
					terminate("}");
					terminate("} /* _try_dispatch_translation_ "
							+ currentTryBlock + " */");
				}
			...
			for (TryBlock.Handler h = t.handler; h != null; h = h.next) {
				...
				if (ctx.cExceptions) {
					...
					if (DEBUG) {
						w
								.print("\tubprintf(\"Jumping to exception handler of exception %x of type ");
						ptype(ex.getBlueprint(null));
						terminateAndIndent("...\\n\",cur_exc); ");
					}
					terminateAndIndent("\tcur_exc=0;");
				...
				if (ctx.noCppExceptions) {
					...
					if (h.bp.equals(ctx.throwableBP)) {
						terminateAndIndent(this.finallyReplacement("c", null,
								false));
					} else {
						w.print("    catch (");
						ptype(h.bp);
			...
			if (ctx.noCppExceptions) {
				...
				// generate last resort jump (if no catch clause caught the
				// exception)
	...
	/**
	 * Coerce an expression of type `from' to type `to'. This may or may not
	 * involve a cast. If a cast is needed, print '((T) ' and return the string
	 * needed to complete the expression '('. Otherwise, print nothing and
	 * return the empty string.
	 * 
	 * @param from
	 *            type to convert from (may be null)
	 * @param to
	 *            type to convert to (may be null)
	 * 
	...
	String coerce(S3Blueprint from, S3Blueprint to) {
		...
		if (from == to)
		...
		else if (from != null && from.isScalar()
				&& !(to instanceof S3Blueprint.Primitive)) {
			if (to == null || !to.isScalar()) {
				// w.print("/* !!! passing from scalar " +
				// CxxFormat.format(from) + " - " + from + " to nonscalar " +
				// CxxFormat.format(to) + " - " + to + " */");
				w.print("((");
				w.print(CxxFormat.formatCArray((S3Blueprint.Array) to));
				w.print(" *) /*");
				w.print(CxxFormat.format(((S3Blueprint.Array) to)
						.getComponentBlueprint()));
				w.print("*/");
				return ")";
			}
			...
			if (from == null || !from.isScalar()) {
				// w.print("/* !!! passing from nonscalar " + (from==null ? "" :
				// CxxFormat.format(from)) + " - " + from + " to scalar " +
				// CxxFormat.format(to) + " - " + to + "*/");
				w.print("((HEADER *)");
				return ")";
			}
			...
			needCast = false;
				...
				|| to == ctx.OopBP || from == ctx.VM_AddressBP
				|| to == ctx.VM_AddressBP || from == ctx.VM_WordBP
				|| to == ctx.VM_WordBP || (to instanceof S3Blueprint.Array)
				|| (to instanceof S3Blueprint.Primitive))
			needCast = true;
	...
	/*
	 * Handle SpecificationIR that is (mostly) shared with the interpreter
	 * generator
	 */
	...
	public void visit(BlueprintReference v) {
		if (v.isConcrete()) {
			w.print("(HEADER *)(&" + J2cFormat.formatBP(v.blueprintValue())
					+ ")");
			// w.print("/* unformatted bp is " + v.blueprintValue() +
			// " (case1)*/");
		} else {
	...
	public void visit(RootsArrayBaseAccessExp v) {
		...
		// w.print( "(HEADER *) ( ((e_s3_core_domain_S3Domain*)&"+
		// CxxFormat.format(dom) +
		// ")->" + rootsField + "->values)" );
		...
		w.print("(HEADER *)  ((e_s3_core_domain_S3Domain*)&"
				+ CxxFormat.format(dom) + ")->" + rootsField);
	}
	...
	public void visit(RootsArrayOffsetExp v) {
		 ...
		 * w.print( "&((HEADER *) ( ((e_s3_core_domain_S3Domain*)&"+
		 * CxxFormat.format(dom) + ")->" + rootsField +
		 * "->values["+v.idx+"])) - &((HEADER *)  ((e_s3_core_domain_S3Domain*)&"
		 * + CxxFormat.format(dom) + ")->" + rootsField+")");
		 */
		...
		w.print("(jint) (&(( (HEADER **) (((e_s3_core_domain_S3Domain*)&"
				+ CxxFormat.format(dom) + ")->" + rootsField + "->values)) ["
				+ v.idx + "])) " +

				"- (jint)(  ((e_s3_core_domain_S3Domain*)&"
				+ CxxFormat.format(dom) + ")->" + rootsField + ")");
	}
	...
	public void visit(J2cNumeric v) {
		if (v.isConcrete()) {
			...
			case TypeCodes.LONG:
				suffix = "LL";
				printable = "(jlong) 0x"
						+ Long.toHexString(((Long) printable).longValue());
				break;
			...
			case TypeCodes.DOUBLE:
				...
				if (Double.isNaN(d))
					...
					w.print("-HUGE_VAL");
				else if (Double.doubleToRawLongBits(d) == Double
			...
			case TypeCodes.FLOAT:
				...
				if (Float.isNaN(f))
					...
					w.print("((float) -HUGE_VAL)");
				else if (Float.floatToRawIntBits(f) == Float
	...
	public void visit(J2cValue v) {
		if (v.getName() != null) {
			if (ctx.gcFrames && mc.everLive.get(v.number)
					&& !currentCallsiteIgnoresSafePoints)
				w.print("frame->");
			// w.print("frame.");
	...
	public void visit(J2cLookupExp exp) {
		...
		w.print("))");
		w
				.print("( (e_ovm_core_services_memory_VM_1Address **) ( ((e_s3_core_domain_S3Blueprint*) (");
		visitAppropriate(exp.bp);
		w.print("))");
		String table = ("ovm_j2c"
				+ Character.toUpperCase(exp.tableName.charAt(0)) + exp.tableName
				.substring(1, exp.tableName.length()));
	...
	public void visit(JValueExp exp) {
		if (outerValue.isPrimitive() || outerValue.isWidePrimitive()) {
			...
			if (outerValue instanceof J2cArray) {
				w.print(CxxFormat.formatCArray((S3Blueprint.Array) outerValue
						.getBlueprint(dom)));
				w.print(" *");
	...
	public void visit(BinExp exp) {
		...
		if ((exp.operator == "/" || exp.operator == "%")
				...
				&& ((J2cValue) exp.rhs).isConcrete() && exp.rhs.intValue() == 0) {
			/*
			 * Work around apple gcc-3.3 bug: it gets stuck trying to compile
			 * division by 0. This code will have to change if we ever want to
			 * do hardware division by zero checks.
			 */
	...
	public void visit(ReinterpretExp exp) {
		...
		if (outerValue.isReference()
				&& (exp.before instanceof J2cFloat || exp.before instanceof J2cDouble)) {
			w.print("(( ");
			if (outerValue instanceof J2cArray) {
				w.print(CxxFormat.formatCArray((S3Blueprint.Array) outerValue
						.getBlueprint(dom)));
			...
			if (outerValue instanceof J2cArray) {
				...
				assert (outerValue instanceof J2cReference);
				if ((exp.before instanceof J2cReference)
						&& !(exp.before instanceof J2cArray)) {
	...
	public void visit(AssignmentExp exp) {
		...
		if (exp.dest instanceof InternalReference)
			...
			compileWithType((J2cValue) exp.src, ((J2cValue) exp.dest)
					.getBlueprint(ctx.domain));
	}
	...
	public void visit(CondExp exp) {
		...
		if (exp.lhs.isReference() && exp.rhs.isReference()) {
			if (false) {
				w.println("/*");
				w.println("((J2cValue) exp.lhs).getBlueprint(ctx.domain) "
						+ ((J2cValue) exp.lhs).getBlueprint(ctx.domain));
				w.println("((J2cValue) exp.rhs).getBlueprint(ctx.domain) "
						+ ((J2cValue) exp.rhs).getBlueprint(ctx.domain));
				w.println("((J2cValue) exp.lhs).isConcrete() "
						+ ((J2cValue) exp.lhs).isConcrete());
				w.println("((J2cValue) exp.rhs).isConcrete() "
			...
			// && !((J2cValue) exp.lhs).isConcrete() && !((J2cValue)
			// exp.rhs).isConcrete()
			// this breaks constant strings comparison (user domain constant
			// strings can move)
			// and it was a workaround for non-working getReferenceAssertions -
			// which is now fixed
			// delete the comment when sufficiently confident that it is now
			// correct
			) {
		...
		if (bar) {
			int al = SpecInstantiation.getReferenceAssertions(ctx,
					(J2cValue) exp.lhs);
			int ar = SpecInstantiation.getReferenceAssertions(ctx,
					(J2cValue) exp.rhs);

			S3Blueprint lbp = ((J2cValue) exp.lhs).getBlueprint(ctx.domain);
			S3Blueprint rbp = ((J2cValue) exp.rhs).getBlueprint(ctx.domain);

			// !! implicitly assumed here the barrier does not have safe points,
			// does not
			// throw exceptions, ... - otherwise, startCall, endCall will be
			// needed
			w.print(J2cFormat.format((exp.operator == "==") ? ctx.acmpeqBarrier
					: ctx.acmpneBarrier));
			w.print("(");
			visitAppropriate(ctx.CSAvalue);
			w.print(", ");
	...
	public boolean cannotThrowException(Value target) {
		...
		if (exceptionNonThrowers == null) {
			...
			exceptionNonThrowers.add(J2cValue
					.makeSymbolicReference("needs_init"));
			exceptionNonThrowers.add(J2cValue
					.makeSymbolicReference("is_subtype_of"));
			exceptionNonThrowers.add(J2cValue
					.makeSymbolicReference("subtype_of_scalar"));
			exceptionNonThrowers.add(J2cValue
					.makeSymbolicReference("array_store_valid"));
			exceptionNonThrowers.add(J2cValue
					.makeSymbolicReference("ARRAY_STORE_INVALID"));
		}
	...
	public void visit(InvocationExp exp) {
		...
		if (ctx.cExceptions
				&& (J2cValue.makeSymbolicReference("j2c_throw") == exp.target)) {
			...
			if (tryBlocks.size() == 0) {
				...
				try {
					int currentTryBlock = ((TryBlockInfo) tryBlocks
							.lastElement()).code;
					w.print(currentTryBlock);
		...
		String rest = (outerValue == null ? "" : coerce(exp.rt, outerValue
				.getBlueprint(ctx.domain)));
	...
	int computeSavedVars() {
		...
		for (int i = liveRefs.nextSetBit(0); i != -1; i = liveRefs
				.nextSetBit(i + 1)) {
	...
	void startCall(boolean isSafePoint, S3Blueprint bp, S3Method m) {
		...
		if (m != null) {
			targetBP = (S3Blueprint) m.getDeclaringType().getDomain()
					.blueprintFor(m.getDeclaringType());
		}
	...
	void startCall(boolean isSafePoint, S3Blueprint bp, S3Method m,
		...
		if (currentCallsiteIgnoresExceptions) {
			...
			if ((m != null) && (targetBP != null)) {
				...
				if (PragmaAssertNoExceptions.descendantDeclaredBy(m
						.getSelector(), targetBP) != null) {
		...
		if (ctx.cExceptions && !ctx.safePointMacros
				&& currentCallCanThrowExceptions) {

			// before each call, exceptions must be cleared
			// (there can be a context switch that could otherwise clobber the
			// exception
			// status)

			// after each call, must check of exception and if there is any,
			// jump
		...
		if (TEST_LIVENESS) {
			...
			if (!isSafePoint)
				return;
			w.print("({ ");
			boolean ret = (bp != null && bp.getType().getUnrefinedName() != TypeName.Primitive.VOID);
			if (ret) {
				ptype(bp);
				w.print(" _r =");
			}
			...
			for (int i = 0; i < allVars.length; i++) {
				if (allVars[i] != null
					...
					localsList.append(" *) 0xffffffff /*" + allVars[i].number
							+ "*/;");
				}
			}
			if (ret)
	...
	/**
	 * Instructions whose behavior isn't completely described by SpecIR
	 */
	void jump(BBSpec to) {
	...
	public void visit(Instruction.POLLCHECK i) {
		...
		terminateAndIndent("if ("
				+ PollcheckManager.getSettings().fastPathInC() + ") {");
		w.print("\t");
	...
	public void visit(Instruction.NULLCHECK i) {
		...
		if (inputs[0] instanceof J2cReference) {
			...
			if (false) {
				terminateAndIndent("/* NC: r.kind=" + r.kind + " r.number="
						+ r.number + " m.isVirtual=" + m.isVirtual()
						+ " m.isConstructor=" + m.isConstructor() + " r.name="
						+ r.name + " */");
				terminateAndIndent("/* thisVar = " + thisVar + "*/");
				if (thisVar != null) {
					terminateAndIndent("/* NC: thisVar.kind=" + thisVar.kind
							+ " thisVar.number=" + thisVar.number
							+ " thisVar.name=" + thisVar.name + " */");
				}
			}

			// FIXME: a lot of guessing is involved in this condition
			// FIXME: this doesn't allow to remove nullchecks in inlined methods
			if ((thisVar != null) && (r.kind == J2cValue.LOCAL_VAR)
					&& (r.number == thisVar.number)
					&& (m.isVirtual() || m.isConstructor())) {
			...
			if (inputs[0] instanceof J2cNull) {
				...
				terminateAndIndent("/* NC: is neither J2cReference, nor J2cNull: "
						+ inputs[0] + "*/");
				throw new Error(
						"NULLCHECK input is neither J2cNull, nor J2cReference");
			}
		}
