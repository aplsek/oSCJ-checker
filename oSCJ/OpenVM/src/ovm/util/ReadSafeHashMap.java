...
/**
 * A hashed {@link Map} in which {@link #get} operations need not
 * synchronize with {@link #put}.<p>
 *
 * This is a modified copy of HashMap from classpath-0.4.  In many
 * ways, it is more efficent than our current implementation of
 * HashMap, and it provides an important additional benifit.
 * Multiple gets can execute concurrently with a single put.
 * (At least on a uniprocessor).<p>
 *
 * This is actually fairly straightforward.
 * <ul>
 * <li> we are careful to read buckets once in get
 * <li> we leave the old buckets array unmodified in rehash
 * </ul>
 * But, it does force us to allocate fresh entries on every rehash.<p>
 *
 * Adapted from an old version of Classpath's {@link HashMap}.
 *
 * @author Jon Zeppieri
 ...
 */
public class ReadSafeHashMap extends AbstractMap
  implements Map, Cloneable, Serializable
{
   ...
   */
  public ReadSafeHashMap(int initialCapacity)
  {
   ...
   *                                     ! (loadFactor &gt; 0.0)
   */
  public ReadSafeHashMap(int initialCapacity, float loadFactor)
  {
  ...
  {
    HashEntry[] buckets_ = this.buckets;
    int idx = hash(key, buckets_);
    HashEntry e = buckets_[idx];
    while (e != null)
      {
        if (equals(key, e.key))
          return e.value;
  ...
  public Object put(Object key, Object value)
  {
   // HashEntry[] buckets = this.buckets;
    ...
    buckets[idx] = new HashEntry(key, value, buckets[idx]);
    return null;
  }
  ...
  public Object clone()
  {
    ReadSafeHashMap copy = null;
    try
      {
        copy = (ReadSafeHashMap) super.clone();
      }
    ...
    if (keys == null)
      ...
      keys = new AbstractSet()
      {
        ...
        public Iterator iterator()
        {
          ...
          return ReadSafeHashMap.this.iterator(KEYS);
        }

        public void clear()
        {
          ReadSafeHashMap.this.clear();
        }
  ...
  public Collection values()
  {
    if (values == null)
      ...
      values = new AbstractCollection()
      {
        ...
        {
          // Cannot create the iterator directly, because of LinkedHashMap.
          return ReadSafeHashMap.this.iterator(VALUES);
        }
  ...
  public Set entrySet()
  {
    if (entries == null)
      ...
      entries = new AbstractSet()
      {
        ...
        {
          // Cannot create the iterator directly, because of LinkedHashMap.
          return ReadSafeHashMap.this.iterator(ENTRIES);
        }
        ...
        public boolean remove(Object o)
        {
          ...
          if (e != null)
            {
              ReadSafeHashMap.this.remove(e.key);
              return true;
            }
          return false;
        }
   ...
   */
  final int hash(Object key, HashEntry[] _buckets)
  {
    return key == null ? 0 : Math.abs(key.hashCode() % _buckets.length);
  }
  ...
  void putAllInternal(Map m)
  {
	...
	Object key = e.getKey();
	int idx = hash(key, buckets);
	buckets[idx] = new HashEntry(key, e.getValue(), buckets[idx]);
      }
  }
   ...
   */
  private HashEntry[] rehash()
  {
    HashEntry[] oldBuckets = buckets;

    int newcapacity = (buckets.length * 2) + 1;
    threshold = (int) (newcapacity * loadFactor);
    HashEntry[] _buckets = new HashEntry[newcapacity];
    ...
    for (int i = oldBuckets.length - 1; i >= 0; i--)
      {
        ...
        while (e != null)
          {
	    int idx = hash(e.key, _buckets);
	    _buckets[idx] = new HashEntry(e.key, e.value, _buckets[idx]);
	    e = e.next;
          }
      }
...
//        throw new ConcurrentModificationException();
    ...
    public void remove()
    {
      ...
      ReadSafeHashMap.this.remove(last.key);
      last = null;
      knownMod++;
    }
  }
