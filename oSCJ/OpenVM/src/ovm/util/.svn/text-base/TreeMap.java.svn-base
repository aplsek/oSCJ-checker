  ...
  implements SortedMap, Cloneable
{
   ...
   * <p>
   * (see Treeset.TreeSet(SortedSet), if available)
   *
   * @param k     the iterator over the sorted keys
   * @param count the number of nodes to insert
   */
  final void putKeysLinear(Iterator k, int count)
  {
    fabricateTree(count);
    Node node = firstNode();
