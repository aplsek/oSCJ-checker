...
Minor modifications for OVM by C. Grothoff, H. Yamauchi
*/
package ovm.util;
 ...
 * @author        Ben L. Titzer
 * @version       $Id: ArrayList.java,v 1.6 2007/06/03 01:25:47 baker29 Exp $
  ...
  implements List, Cloneable, Serializable
{
  ...
  public Object[] toArray(Object[] array)
  {
	...
	throw new IllegalArgumentException("toArray must be given an array of size() elements in OVM!");
    if (array.length > size)
      array[size] = null;
    System.arraycopy(data, 0, array, 0, size);
    return array;
    ...
    throws IOException, ClassNotFoundException
  {
    ...
    // This iterator definition breaks the specification of this class
    // as per the java.util.ArrayList specification because
    // it doesn't detect concurrent modifications.
    // Some uses of this class in OVM (eg. Subtyping.java) rely on this
	...
	public void remove() {
            // if we're not removing the end element then shift everything
            // up to fill the gap
            // Note: next() has already incremented index so we need to
            // decrement it again
            if (--index != --size) {
                System.arraycopy(data, (index + 1), data, index, (size - index));
            }
