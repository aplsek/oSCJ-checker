...
It has been modified slightly to fit the OVM framework. It has also
been fixed.
 ...
 * @modified      $Id: Bucket.java,v 1.3 2003/03/31 19:56:29 jv Exp $
 */
class Bucket extends OVMBase
{

    /** sentinel value to represent no entry was in the bucket. We can't
     * use <code>null</code> because that is a valid value.
     */
    static final Object NO_ENTRY = new Object();
    ...
    Object add(Node newNode)  {
        ...
        Node it = first;
        Node prev = null;
         // if the list is empty (the ideal case), we make a new single-node list
        if (it == null) {
            first = newNode;
            return NO_ENTRY;
        }
        else  {
            // otherwise try to find where this key already exists in the list,            // and if it does, replace the value with the new one 
            // and return the old one)
            while (it != null) {
                oKey = it.getKey();
                if (equals(oKey,oTestKey)) {
    ...
    /**
     * return the Map.Entry which the supplied key is a part of, if such a 
     * Map.Entry exists, null otherwise
     *
