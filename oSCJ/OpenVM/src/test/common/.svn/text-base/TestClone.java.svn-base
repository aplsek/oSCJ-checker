...
public class TestClone 
	...
	check_condition(c != a);
	check_condition(c.i == 4);
	check_condition(c.f == 0.4f);
	check_condition(c.s == 4);
	check_condition(c.l == 3L);
	check_condition(c.bo == true);
	check_condition(c.b == 1);
	check_condition(c.d == 3.0);
    }
	...
	int[] c = (int[])a.clone();
	check_condition(c != a, "not distinct");
	check_condition(c.length == a.length, "length mismatch");
	for(int i = 0; i < a.length; i++) {
	    check_condition(a[i] == c[i], "content mismatch");
	}
    }
	...
	int[][] c = (int[][])a.clone();
	check_condition(c != a, "not distinct");
	check_condition(c[0] == a[0], "not shallow"); // means a shallow copy
	check_condition(c.length == a.length, "length mismatch");
	for(int i = 0; i < a.length; i++) {
	    check_condition(a[i].length == c[i].length, "length mismatch");
	    for(int j = 0; j < a[i].length; j++) {
		check_condition(a[i][j] == c[i][j], "content mismatch");
	    }
	}
    }
