...
/**
 * The entry point for executing a Realtime Java (RTSJ) supporting user-domain
 *
...
public final class RealtimeLauncher {
     ...
     * It also means that we cannot reference any String literal before class
     * initialization is enabled otherwise we will crash due to nested NPE's.
    ...
    static void main(String[] notUsed) {
        ...
        try {
            ...
            java.lang.RTJavaVirtualMachine.init(); 

            // now we can handle Strings
            String[] args =
                LibraryImports.getCommandlineArgumentStringArray();
            ...
            for(int i = 0; i < args.length; i++) {
                ...
                if (args[i].equals("-dotests")) {
                    runTests = true;
                    if (!supportTests) {
                      throw new RuntimeException("Tests are now disabled in code.\n");
                    }
                    break;
                }
            }
            ...
            // launch the RT-JVM
            java.lang.RTJavaVirtualMachine.getInstance().run();
        }
        catch(Throwable t) {
	    LibraryImports.enableClassInitialization(); // may be necessary -HY
            ...
            LibraryImports.printString(
                "RT-LAUNCHER: Uncaught user-domain exception: ");
