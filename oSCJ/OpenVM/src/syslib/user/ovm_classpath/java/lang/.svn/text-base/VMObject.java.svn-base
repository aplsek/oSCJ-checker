...
final class VMObject
{
   ...
   */
    static void notify(Object o) throws IllegalMonitorStateException {
        LibraryImports.monitorNotify(o);
    }
   ...
   */
    static void notifyAll(Object o) throws IllegalMonitorStateException {
        LibraryImports.monitorNotifyAll(o);
    }
  ...
  static void wait(Object o, long ms, int ns)
      ...
      // the underlying monitor implementation treats 0 as don't wait at all
      // so we need to convert if the timeout is zero.
      if (ms == 0 && ns == 0) {
          if (!LibraryImports.monitorWait(o)) {
              // interrupted while waiting so clear and throw
              JavaDispatcher.getInstance().getCurrentThread().interrupted = false;
              throw new InterruptedException();
          }
      }
      else {
          if (!LibraryImports.monitorTimedWait(o, ms, ns)) {
              // interrupted while waiting so clear and throw
              JavaDispatcher.getInstance().getCurrentThread().interrupted = false;
              throw new InterruptedException();
          }
