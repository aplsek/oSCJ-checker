    ...
    static class Low implements Runnable {
        public void run() {   
            ...
            for (int i = 0; i < arg.REPEAT; i++)
                switch (CURRENT_MODE) {
                    case RAW: list.r_get(arg.POS); break;
                   case SYNC:list.s_get(arg.POS);   break;
                  case TLF: list.t_get(arg.POS);
                }
            // Run
            while (!stop) {
...
//                p("  " + results[CURRENT_MODE].value/1000);
            }
//            pln("");
