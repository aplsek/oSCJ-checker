 ...
 * 
 * @Scope("FooMission")  
 * RunsIn is implied to be FooMission 
 *  class FooMission
 *
 ...
 * initialize(), but that's just extra work. I wouldn't bother doing it
 * unless not restricting it breaks in some case (which it may, I have
...
public class ScopeVisitor<R, P> extends SourceVisitor<R, P> {
    ...
    public R visitClass(ClassTree node, P p) {
        ...
        */
        ...
        try {
            ...
            if (runsIn != null && scope == null) { throw new ScopeException(
            ...
            if (runsIn == null && scope == null) {
                /** TODO: The correct behavior for visiting unannotated
                *       classes is to make sure the class doesn't mention any annotated
            ...
            Utils
                .debugPrintln("Seen class " + t.getQualifiedName() + ": @Scope(" + scope + ") @RunsIn(" + runsIn + ")");
       ...
       // System.out.println("\n member select:" + node.toString());
        ...
        try {
            ...
            if (elem.getKind() == ElementKind.FIELD) {
                ...
                //System.out.println("   type - kind:" + elem.asType().getKind());
                String typeScope = context.getScope(type);
                //System.out.println("   typ scope :" + typeScope);
                //System.out.println("   scope elem :" + scope((VariableElement) elem));
    ...
    private void checkMethodInvocation(MethodInvocationTree node, P p) throws ScopeException {
        ...
        if (isMemoryAreaType(type) && ("executeInArea".equals(methodName) || "enter".equals(methodName))) {
            ...
            case IDENTIFIER:
                ...
                //System.out.println("argument var "  + (VariableElement) TreeUtils.elementFromUse((IdentifierTree) arg));
            ...
            case NEW_CLASS:
                ...
                argRunsIn = context.getRunsIn((TypeElement) ctor.getEnclosingElement());
                break;
            default:
                report(Result.failure("bad.enter.parameter"), arg);
                return;
            ...
            if (!(runsIn.equals(currentAllocScope()) || (Utils.isAllocFree(method, ats) && ScopeTree.isParentOf(
                ...
                report(Result.failure("bad.method.invoke", runsIn, currentAllocScope()), node);
            }
    ...
    public R visitNewClass(NewClassTree node, P p) {
        try {
            ...
            // System.err.println(nodeClassScope + " =? " + currentAllocScope());
    ...
    public R visitTypeCast(TypeCastTree node, P p) { 
        ...
        if (exprKind == Kind.NEW_CLASS) {
            ...
            exprScope = Utils.annotationValue(expressionType.getAnnotationMirrors(), "javax.safetycritical.annotate.Scope");
        ...
        if (exprScope != null)
            if (typeScope == null) {
                ...
                if (!exprScope.equals(currentAllocScope()))
                    report(Result.failure("scope.cast", exprScope, currentAllocScope()), node);
            }
    ...
    private boolean escapeTypes(Tree type) {
        if (type.toString().startsWith("float")) return true;
        else  if (type.toString().startsWith("byte"))  return true;
        else  if (type.toString().startsWith("int"))  return true;
        else  if (type.toString().startsWith("double"))  return true;
        else  if (type.toString().startsWith("short"))  return true;
        else  if (type.toString().startsWith("char"))  return true;
        else  if (type.toString().startsWith("boolean"))  return true;
        else  if (type.toString().startsWith("long"))  return true;
        return false;
    }
    ...
    private void checkVariable(VariableTree node, P p) throws ScopeException {
        ...
        if (nodeTypeTree.getKind() == Kind.PRIMITIVE_TYPE) { return; }
        ...
        if (isStatic(var.getModifiers())) {
            ...
            if (varScope != null && !ScopeTree.isParentOf(currentAllocScope(), varScope)) {
                ...
                report(Result.failure("bad.variable.scope", atf.fromTypeTree(node.getType()), currentAllocScope()),
                    node);
        ...
        if (node.getInitializer() != null) {
            //System.out.println("checking assignmenet!!!!");
    ...
    private boolean isPrivateMemory(VariableTree node, P p) {
        VariableElement var = TreeUtils.elementFromDeclaration(node);
        if (isPrivateMemory(var.asType())) return true;
        return false;
    }
     ...
     * 
     * 2. and then that the @DefineScope is valid. 
     * think the syntax will usually be:
     * something like 
     *      @DefineScope(name=x, parent=y) 
     *       PrivateMemory mem = ManagedMemory.getCurrentManagedMemory() 
     * (actually this method returns
     * ManagedMemory so this requires a little more thinking). We have to check
     * that x is the current allocation context where it is called (determined
     * by currentAllocContext()).
     * 
     * @param node
     * @param p
     */
    private void checkDefineScope(VariableTree node, P p) {
        ...
        if (scopeDef != null) {
            if (!ScopeTree.hasScope(scopeDef.name)) {
                ...
                report(Result.failure("checker.bug", "ERROR: ScopeVisitor.checkDefineScope error behavior"), node);
            }
            if (!currentAllocScope().toString().equals(scopeDef.name) )
                report(Result.failure("bad.enterPrivateMem.defineScope",currentScope.toString(),scopeDef.name), node);
            ...
            if (! ScopeTree.get(scopeDef.name).equals(scopeDef.parent) )
                ...
                report(Result.failure("bad.enterPrivateMem.defineScope.parent",currentScope.toString(),scopeDef.name), node);
        
        } else 
            report(Result.failure("bad.enterPrivateMem.no.defineScope",currentScope.toString()), node);
    ...
    private DefineScope getDefineScope(VariableTree node) {
        ...
        for (AnnotationMirror ann : var.getAnnotationMirrors()) {
            if (ann.getAnnotationType().toString().equals("javax.safetycritical.annotate.DefineScope")) {
                ...
                for (Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : ann.getElementValues()
                    .entrySet()) {
    ...
    private boolean isPrivateMemory(TypeMirror asType) {
        if (asType.toString().equals("javax.safetycritical.PrivateMemory")) return true;
        if (asType.toString().equals("javax.safetycritical.ManagedMemory")) return true;
        return false;
    }
    ...
    private String scope(VariableElement var) throws ScopeException {
        ...
        if (varScope != null) { return varScope; }
    ...
    private String varScope(VariableElement var) throws ScopeException {
        ...
        if (exprType.getKind() == TypeKind.DECLARED) {
            ...
            return context.getScope((TypeElement) ((AnnotatedDeclaredType) exprType).getUnderlyingType().asElement());
        }
        return null;
    }
        ...
        for (AnnotationMirror mirror : type.getAnnotationMirrors()) {
            if (TypesUtils.isDeclaredOfName(mirror.getAnnotationType(), "javax.safetycritical.annotate.RunsIn")) {
                ...
                for (Entry<? extends ExecutableElement, ? extends AnnotationValue> e : vals.entrySet()) {
                    if ("value".equals(e.getKey().getSimpleName().toString())) { return e.getValue().getValue()
                        .toString(); }
                }
            }
        }
    ...
    // Returns the variable corresponding to the left-hand side of an assignment. If the LHS is an array access, it
    ...
    private ExpressionTree simplify(ExpressionTree exprTree) {
        while (true) {
            ...
            case ASSIGNMENT:
                ...
                break;
            /*
             * case TYPE_CAST: exprTree = ((TypeCastTree) exprTree).getExpression(); break;
             */
            default:
                return exprTree;
            }
    ...
    private boolean checkAssignment(Tree varTree, ExpressionTree exprTree, Tree errorNode) throws ScopeException {
        ...
        if (isPrimitiveExpression(exprTree)) { return true; // primitive assignments are always allowed
        }
        
        VariableElement varElem = lhsToVariable(varTree);
        if (varElem == null) { return true; // annotation assignments are irrelevant
        }
        ...
        if (exprKind == Kind.NULL_LITERAL || exprKind == Kind.STRING_LITERAL) {
            ...
            if (currentAllocScope() != null &&
                ...
                report(Result.failure("bad.assignment.scope", currentAllocScope(), varScope), errorNode);
            }
            ...
            if (checkForPrivateMemAssignementError(var1, var2)) {
                ...
                report(Result.failure("bad.assignment.private.mem", exprScope, varScope), errorNode);
            }
        ...
        if (!isLegal) {
            report(Result.failure("bad.assignment.scope", exprScope, varScope), errorNode);
        }
    ...
    private String getScopeDef(ExpressionTree e) throws ScopeException {
        ...
        case METHOD_INVOCATION:
            ...
            // ImmortalMemory.instance(). We should support MemoryArea.getMemoryArea() to the degree that it is
            // possible.
            break;
        case IDENTIFIER:
        case MEMBER_SELECT:
    ...
    private boolean isPrimitiveExpression(ExpressionTree expr) {
       ...
       // System.out.println("kind:" + atf.fromExpression(expr).getKind().isPrimitive());
