...
class MyHandlerIterator extends PeriodicEventHandler {
    ...
    public void handleAsyncEvent() {
        ...
        @Scope("Unknown")
        MyLinkedList mylist = mission.list; // LOCAL INFERENCE .....
                                            // ///mission.getList();
        this.myList = mission.list; // WE PROPOSE @LivesIN to have
                                    // mission.getList();
        Iterator iterator = this.myList.iterator(); // returns object in the
                                                    // current scope!!!!
        @Scope("Unknown")
        Node node = iterator.getNext(); // should return the reference to Node
                                        // living in @Scope("Mission")
        node.method();
    }
}
